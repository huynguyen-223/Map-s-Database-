...
----------------------------------------- BO SUNG PROD CHO GIAO DIEN
if OBJECT_ID ('INSERT_TUYEN_XE_BUS','P') is not null
drop proc INSERT_TUYEN_XE_BUS
GO
create proc INSERT_TUYEN_XE_BUS
	@MA_TUYEN_XE VARCHAR(4),
	@STT INT
AS
BEGIN
	INSERT TUYEN_XE_BUS VALUES(@MA_TUYEN_XE);
	INSERT CHUYEN_TAU_XE VALUES (@MA_TUYEN_XE,@STT)
END
GO
------ INSERT_CHUYEN_TAU_XE
if OBJECT_ID ('INSERT_CHUYEN_TAU_GHE_TRAM','P') is not null
drop proc INSERT_CHUYEN_TAU_GHE_TRAM
GO
create proc INSERT_CHUYEN_TAU_GHE_TRAM
	@MA_TUYEN_XE VARCHAR(4),
	@STT INT,
	@MA_GA VARCHAR(7),
	@STT_TRAM_DUNG INT,
...
AS
BEGIN
	INSERT CHUYEN_TAU_GHE_TRAM VALUES (@MA_TUYEN_XE ,@STT ,@MA_GA,@STT_TRAM_DUNG,@GIO_GHE,@GIO_DI)
END
GO
------SELECT_HANH_KHACH
 if OBJECT_ID ('SELECT_HANH_KHACH','P') is not null
drop proc SELECT_HANH_KHACH
GO
create proc SELECT_HANH_KHACH
	@MA_HANH_KHACH VARCHAR(8)
AS
BEGIN
	SELECT * FROM dbo.HANH_KHACH WHERE MA_HANH_KHACH = @MA_HANH_KHACH;
END
GO
------SELECT_ALL_HANH_KHACH
 if OBJECT_ID ('SELECT_ALL_HANH_KHACH','P') is not null
drop proc SELECT_ALL_HANH_KHACH
GO
create proc SELECT_ALL_HANH_KHACH
...
GO
------SELECT_ACCOUNT
 if OBJECT_ID ('SELECT_ACCOUNT','P') is not null
drop proc SELECT_ACCOUNT
GO
create proc SELECT_ACCOUNT
	@userName VARCHAR(30),
	@passWord VARCHAR(30)
AS
BEGIN
	SELECT * FROM dbo.ACCOUNT WHERE USERNAME = @userName AND PASSWORD_ = @passWord ;
END
GO
-------------------------------------------------END-----------------------------------------------------------------------------